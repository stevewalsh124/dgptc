...
# this is from 1D_toy
r <- 2000 #number of runs
n <- 1000 #number of points per run
x <- seq(0,1,length.out=n)
w <- x^2

pdf(paste0("pdf/1D_toy_hetGP_",r,"_",n,".pdf"))
...
for (rr in 1:10) {
  ...
  plot(x, diag(Sigma_true), col="blue", type="l", ylim = c(0,max(c(diag(Sigma_true),model$Lambda, sqrt(model$Lambda)))))
  lines(x, model$Lambda)
  lines(x, sqrt(model$Lambda), col="red")
}
