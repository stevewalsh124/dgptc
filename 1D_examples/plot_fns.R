...
plot.krig <- function(fit, zz=fit$mean, Y=parent.frame()$Y, precs_pred=parent.frame()$precs_pred){
  ...
  lines(fit$x_new, zz+2*sqrt(1/(nrun*precs_pred)), col=3, lwd=1.5)
  lines(fit$x, y_hi, lwd=1.5, lty=2)
...
plot.true <- function(fit, S_e = fit$Sigma_hat){
  ...
  emp_cover <- round(mean(y_hi > lb & y_hi < ub),3)
  emp_cover99 <- round(mean(y_hi > lbb & y_hi < ubb),3)
